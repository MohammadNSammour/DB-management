...
namespace Rabat_Charitable_Association
{
    public partial class Form5 : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("من فضلك ادخل معلومات صحيحة للمال", "خطأ في الإدخال", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            string query = "INSERT INTO donors (ID, fullname, cashgaven, phonenumber, paydate, notes) VALUES (@ID, @fullname, @cashgaven, @phonenumber, @paydate, @notes)";
            try
            {
                using (OleDbConnection connection = new OleDbConnection(connection_path))
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@cashgaven", cash);
                        command.Parameters.AddWithValue("@phonenumber", textBox4.Text);
                        command.Parameters.AddWithValue("@paydate", dateTimePicker1.Value);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(textBox5.Text, out int cash))
            {
                MessageBox.Show("من فضلك ادخل معلومات صحيحة للمال", "خطأ في الإدخال", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            string query = "UPDATE donors SET fullname = @fullname, cashgaven = @cashgaven, phonenumber = @phonenumber, paydate = @paydate, notes = @notes WHERE ID = @ID";
            try
            {
                using (OleDbConnection connection = new OleDbConnection(connection_path))
                {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string query1 = "SELECT * FROM donors ORDER BY ID";
            try
            {
                using (OleDbConnection connection = new OleDbConnection(connection_path))
                {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            chart1.Visible = true;
            chart2.Visible = true;
            string query = "SELECT paydate, SUM(cashgaven) AS TotalCash" +
                        " FROM donors" +
                        " WHERE cashgaven IS NOT NULL AND paydate IS NOT NULL" +
                        " GROUP BY paydate" +
                        " ORDER BY paydate";
            try
            {
                using (OleDbConnection connection = new OleDbConnection(connection_path))
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, connection))
                    {
                        using (OleDbDataReader reader = command.ExecuteReader())
                        {
                            chart1.Series.Clear();
                            chart2.Series.Clear();
                            Series series = new Series("المال المقدم")
                            {
                                ChartType = SeriesChartType.Column
                            };
                            Series series1 = new Series("المال المقدم")
                            {
                                ChartType = SeriesChartType.Pie
                            };
                            while (reader.Read())
                            {
                                if (DateTime.TryParse(reader["paydate"].ToString(), out DateTime paydate) &&
                                    double.TryParse(reader["TotalCash"].ToString(), out double totalCash))
                                {
                                    series.Points.AddXY(paydate.ToString("yyyy-MM-dd"), totalCash);
                                    series1.Points.AddXY(paydate.ToString("yyyy-MM-dd"), totalCash);
                                }
                            }
                            chart1.Series.Add(series);
                            chart1.Titles.Clear();
                            chart1.Titles.Add("الاوقات والاموال المسلمة");
                            chart1.ChartAreas[0].AxisX.Title = "التاريخ";
                            chart1.ChartAreas[0].AxisY.Title = "المال الكلي المسلم";
                            chart1.ChartAreas[0].AxisY.LabelStyle.Format = "C";
                            chart2.Series.Add(series1);
                            chart2.Titles.Clear();
                            chart2.Titles.Add("الاوقات والاموال المسلمة");
                            chart2.ChartAreas[0].AxisX.Title = "التاريخ";
                            chart2.ChartAreas[0].AxisY.Title = "المال الكلي المسلم";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                textBox3.Text = selectedRow.Cells["cashgaven"].Value?.ToString();       
                textBox4.Text = selectedRow.Cells["phonenumber"].Value?.ToString();   
                textBox5.Text = selectedRow.Cells["notes"].Value?.ToString();    
                string dateValue = selectedRow.Cells["paydate"].Value?.ToString();
        ...
        {
            ToolTip toolTip = new ToolTip();
            toolTip.SetToolTip(dataGridView1, "جدول بيانات المتبرعين");
        }
        private void chart1_MouseHover(object sender, EventArgs e)
        {
            ToolTip toolTip = new ToolTip();
            toolTip.SetToolTip(chart1, "رسم بياني يوضح المبلغ المسلم في تاريخ معين");
        }
        private void chart2_MouseHover(object sender, EventArgs e)
        {
            ToolTip toolTip = new ToolTip();
            toolTip.SetToolTip(chart2, "رسم بياني يوضح المبلغ المسلم في تاريخ معين على شكل دائري");
        }
